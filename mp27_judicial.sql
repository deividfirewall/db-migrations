-- Active: 1747336191893@@localhost@3309@mp_judicial
...
DELETE FROM `c_tipo_prestamo_sucursal` WHERE c_sucursal_id != 27; 
...
UPDATE t_empenios SET c_tipo_prestamo = 26 WHERE c_tipo_prestamo = 24;    --206 registros
UPDATE t_empenios SET c_tipo_prestamo = 25 WHERE c_tipo_prestamo = 23;    --3 registros
UPDATE t_empenios SET c_tipo_prestamo = 24 WHERE c_tipo_prestamo = 22;    --2068 registros
...
begin ------------------------------------------> t_concentrados
    ...
    UPDATE t_concentrados SET id_gerente  = id_gerente + 270000; 
end t_concentrados;
...
begin ------------------------------------------> r_ro_cg12
    ...
    UPDATE r_ro_cg12    SET    id_operador = 150117 WHERE id_operador = 756    AND sucursal_id = 27;
    UPDATE r_ro_cg12    SET    id_operador = 151478 WHERE id_operador = 755    AND sucursal_id = 27;
...
-- ❌ borramos los empeños que no tienen relacion t_empenios_boleta_relacion (~61 registros)
    ...
    SELECT * FROM t_empenios_boleta_relacion LEFT JOIN t_empenios ON t_empenios_boleta_relacion.t_empenios_id = t_empenios.id WHERE t_empenios.id IS NULL; 
        DELETE t_empenios_boleta_relacion FROM t_empenios_boleta_relacion LEFT JOIN t_empenios ON t_empenios_boleta_relacion.t_empenios_id = t_empenios.id WHERE t_empenios.id IS NULL;
...
begin ------------------------------------------> pignorante_solidario
    ...
    UPDATE u_pignotarios_solidarios SET pignorante_solidario = REPLACE(pignorante_solidario, '  ', ' '); --x8
    ...
    UPDATE u_pignotarios_solidarios SET pignorante_solidario = REPLACE(pignorante_solidario, 'Â‘', 'ª');
    UPDATE u_pignotarios_solidarios SET pignorante_solidario = REPLACE(pignorante_solidario, 'Ã‘', 'ª');
    UPDATE u_pignotarios_solidarios SET pignorante_solidario = REPLACE(pignorante_solidario, 'ªª', 'ª');  --x6
    ...
    UPDATE u_pignotarios_solidarios SET pignorante_solidario = REPLACE(pignorante_solidario, 'MODIFICADO', 'MOD'); 
    -- VERIFICA ESTA CONSULTA ANTES:    DELETE FROM u_pignotarios_solidarios WHERE pignorante_solidario LIKE '% DEJA%' AND LENGTH(pignorante_solidario) < 18;
    ...
    -- Se eliminan los pignorantes_solidarios que no tienen boleta asociada
...
-- Se corrige fechas en 0000-00-00
    UPDATE `t_boleta` SET `fecha_limite_pago` = fecha_movimiento WHERE `t_boleta`.`id` = 10009023; 

------------------------------------------------> h_t_boletas
-- Se corrige fechas en 0000-00-00
    
    UPDATE `h_t_boleta` SET `fecha_limite_pago` = '2021-03-01' WHERE `h_t_boleta`.`id_interno` = 76007; 
    UPDATE `h_t_boleta` SET `fecha_limite_pago` = fecha_movimiento WHERE `h_t_boleta`.`id_interno` = 73663; 
    UPDATE `h_t_boleta` SET `fecha_limite_pago` = fecha_movimiento WHERE `h_t_boleta`.`id_interno` = 42517; 



begin ------------------------------------------> t_comprador   y   t_compra_vitrina    
    UPDATE `t_comprador` SET `rfc`  = null WHERE LENGTH(rfc)  < 4; 
    UPDATE `t_comprador` SET `ife`  = null WHERE LENGTH(ife)  < 4;
    UPDATE `t_comprador` SET `curp` = null WHERE LENGTH(curp) < 4;
    ...
    UPDATE t_comprador SET nombre = REPLACE(nombre, 'Ã‘', 'Ñ');
    UPDATE t_comprador SET nombre = REPLACE(nombre, 'Ãƒ', 'º');
    UPDATE t_comprador SET nombre = REPLACE(nombre, 'Âƒ', 'º');

    UPDATE `t_comprador` SET `nombre` = 'Comprador numero 8' WHERE `t_comprador`.`id` = 8; 
